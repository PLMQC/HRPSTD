report 33001253 "Update Leave Approva B2B"
{
    // version B2BHR1.00.00

    Caption = 'Update Leave Records - Approva';
    ProcessingOnly = true;
        ...
        dataitem("Leave Plan"; "Leave Plan B2B")
        {
            ...
            dataitem("Daily Attendance"; "Daily Attendance B2B")
            {
                ...
                begin
                    ...
                    "Leave Plan"."Posted Status" := "Leave Plan"."Posted Status"::Post;
                    "Leave Plan".MODIFY();
                end;
            }
            ...
            begin
                B2BEmpGRec.RESET();
                B2BEmpGRec.SETRANGE("No.", "Employee No");
                PayrollUsers.GET(USERID());
                if PayrollUsers."Multiple Locations" <> '' then
                    B2BEmpGRec.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
                else
            ...
            begin
                ...
                PayrollUsers.GET(USERID());
                if PayrollUsers."Multiple Locations" <> '' then
                    PostedLeaveAppLine.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
                else
    ...
    begin
        ...
        if (LeavePlan."Leave Type" = LeavePlan."Leave Type"::Leave) and
    ...
    begin
        ...
        PayrollUsers.GET(USERID());
        if PayrollUsers."Multiple Locations" <> '' then
            DailyAttend.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
        else
        ...
        if DailyAttend.FINDFIRST() then
            repeat
                ...
                PayrollUsers.GET(USERID());
                if PayrollUsers."Multiple Locations" <> '' then
                    OffdayDailyAttend.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
                else
                ...
                if OffdayDailyAttend.FINDFIRST() then
                    if DailyAttend."Leave Code" = OffdayDailyAttend."Leave Code" then begin
                        ...
                        PayrollUsers.GET(USERID());
                        if PayrollUsers."Multiple Locations" <> '' then
                            DailyAttendance.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
                        else
        ...
        if DailyAttend.FINDFIRST() then
            repeat
                ...
                if OffdayDailyAttend.FINDFIRST() then begin
                    ...
                    if DailyAttendance.FINDFIRST() then
                        repeat
                            if (DailyAttendance."Attendance Type" = DailyAttendance."Attendance Type"::Present) and not (DailyAttendance."Non-Working") then
                                Flag := true;
    ...
    begin
        ...
        if DailyAttend.FINDFIRST() then
            repeat
                if (DailyAttend."Attendance Type" = DailyAttend."Attendance Type"::Absent) or (DailyAttend."Leave Code" <> '') then begin
                    OffdayDailyAttend.RESET();
                    PayrollUsers.GET(USERID());
                    if PayrollUsers."Multiple Locations" <> '' then
                        OffdayDailyAttend.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
                    ...
                    OffdayDailyAttend.SETFILTER(Date, '>%1', DailyAttend.Date);
                    if OffdayDailyAttend.FINDFIRST() then
                        if (OffdayDailyAttend."Attendance Type" = OffdayDailyAttend."Attendance Type"::Absent) or (OffdayDailyAttend."Leave Code" <> '') then begin
                            DailyAttendance.RESET();
                            PayrollUsers.GET(USERID());
                            if PayrollUsers."Multiple Locations" <> '' then
                                DailyAttendance.SETFILTER("Location Code", PayrollUsers."Multiple Locations")
