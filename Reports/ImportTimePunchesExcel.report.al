report 33001351 "Import Time Punches Excel B2B"
{
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    group("Import from")
                    {
                        ...
                        {
                            ToolTip = 'Choose the Excel file name.';
                        ...
                        {
                            ToolTip = 'Choose the work sheet name.';
    ...
    begin
        ExcelBuf.LOCKTABLE();
        ExcelBuf.DELETEALL();
        //ExcelBuf.OpenBook(ServerFileName, SheetNameGVar);
            ...
            if ExcelBuf.FINDFIRST() then
                repeat
                    ...
                    if (DailyTimePunches."Employee No." <> '') and (DailyTimePunches.Date <> 0D) then begin
                        ...
                        ExcelBuf2.SETRANGE(xlColID, Text0002Txt);
                        ...
                        ExcelBuf2.RESET();
                        ExcelBuf2.SETRANGE("Row No.", I);
                        ExcelBuf2.SETRANGE(xlColID, Text0003Txt);
                        if ExcelBuf2.FINDFIRST() then begin
                        ...
                        ExcelBuf2.RESET();
                        ExcelBuf2.SETRANGE("Row No.", I);
                        ExcelBuf2.SETRANGE(xlColID, Text0004Txt);
    ...
    begin
        if FileNameGVar <> '' then
            ...
            UploadIntoStream(DialogCaptionLbl, '', '', FileNameGVar, NavInstream)
        else
            //ServerFileName := FileMgt.UploadFile(Text006Txt, Text0005Txt);  //SG
            UploadIntoStream(DialogCaptionLbl, '', '', FileNameGVar, NavInstream);

        ValidateServerFileName();
        FileNameGVar := FileMgt.GetFileName(ServerFileName);
    end;
